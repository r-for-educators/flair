...
The core element of the package is an object of the class `decorate_code`.  These objects contain both the natural output of your R code and information to let `knitr` know how display the source code.
...
1. `foo` contains the `decorate_code` object itself, rather than the result of the R code `mean(1:10)`, so  `foo + 5` throws an error.

2. The `decorate_code` output object is being assigned to `foo` rather than printed, so no highlighted code is included in the knitted output.
