...
Here we used `flair` to add pink "flair" to highlight the function we are using to load the data in Python:
...
To create a scatter plot, we can pass the Python data frame into `ggplot` using `py$data_frame` syntax inside a code chunk that this time usees a R engine to make a scatter plot (what can I say, I love `ggolot2`...):
